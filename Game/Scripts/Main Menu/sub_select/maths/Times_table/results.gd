...
#loads values from other scripts - these values are called by the variable "correct_count"
...
func _ready(): #when the scene loads
	...
	var stringCPG = correct_count.get_text() 
	#create a variable named "stringCPG", giving it the value of the correct point amount
	format_string = ("%s") #creates a place holder, essentially a variable for a string
