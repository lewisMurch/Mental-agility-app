...
func _process(delta): #runs the code below every frame
	if main_node.winAnimation == 0: #if the win animation hasn't been played 
		if main_node.correctAns == 1 and inView == 0: #if the correct answer is box 1, and the base-cube isn't in view
			translate(Vector3(-200, -200, -200)) #move the base cube in view
			inView = 1 #say that the base cube is in view
		else: 
			pass #do nothing

	if main_node.winAnimation == 1: #if the user gets cube size 1 correct, play the win animation on that small cube
		...
		if animationRun < 1 and main_node.attempt == 1: #if the selected cube is wrong, play the wrong animation
			translate(Vector3(0, -0.5, 0)) #move slowly down each frame, stopping when it touches the other cube
